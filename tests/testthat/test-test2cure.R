...
test_that("paired two-sample cure-rate test returns correct names and p-value bounds", {
  ...
  res <- test.2.cure(df, time, status, group, type = "paired")
...
test_that("paired test errors when there are not exactly two groups", {
  df3 <- data.frame(
    id     = 1:3,
  ...
  expect_error(
    test.2.cure(df3, time, status, group, type = "paired"),
